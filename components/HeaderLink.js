...
function HeaderLink({Icon, text, avatar, feed, active, hidden}) {
    return <div className={`${hidden && "hidden md:inline-flex"}
    ...
    ${feed
        ...
        {/*underline under the active icon */}
        {active && (
            <span className="hidden lg:inline-flex h-0.5 w-[calc(100%+20px)] bg-black dark:bg-white 
            rounded-t-full" />
        )}
    </div>;
}
