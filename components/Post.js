...
function Post({post, modalPost}) {
  ...
  const deletePost = async () => {
    ...
    setHandlePost(true); //delete posts in realtime
    setModalOpen(false);
  };
  ...
  >
    <div className="flex items-center px-2.5 cursor-pointer">
      ...
      {modalPost ? (
        <IconButton onClick={() => setModalOpen(false)}>
          <CloseRoundedIcon className="mr-auto dark:text-white/75 h-7 w-7" />
        </IconButton>
      ) : (
        <IconButton>
    ...
    {post.input && (
     ...
     mx-2.5 pt-2 text-black/60 dark:text-white/75">
        ...
        {modalPost ?  (
          ...
          >
            {liked ? (
              <ThumbUpOffAltRounded className="-scaled-x-100" />
            ) : (
              <ThumbUpOffAltOutlined className="-scaled-x-100" />
            )}
            <h4>Liked</h4>
          </button>
        )}
